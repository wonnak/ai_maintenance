...
async function getLatestFailedHtml(tcName: string) {
  const baseDir = path.join('htmls', tcName);
...
async function getMatchingSuccessHtml(tcName: string, htmlNumber: string) {
  const successHtmlPath = path.join('htmls', tcName, 'lastSuccessHtmls', `${htmlNumber}.html`);
  try {
...
async function run() {
  const failedTestPath = 'fails/failed_tests.txt';
